"use client";

import { useState } from "react";

import { CloudFilePicker } from "@/components/features/csv-import/CloudFilePicker";
import { GoogleDriveIcon } from "@/components/icons/GoogleDriveIcon";
import { OneDriveIcon } from "@/components/icons/OneDriveIcon";
import type { CloudProvider } from "@/lib/store/cloud-storage";
import { cn } from "@/lib/utils";

const STORAGE_PROVIDERS = [
  { id: "google-drive" as const, label: "Google Drive", Icon: GoogleDriveIcon },
  { id: "onedrive" as const, label: "OneDrive", Icon: OneDriveIcon },
];

interface CloudStorageButtonsProps {
  variant?: "default" | "compact";
  onFileSelect?: (file: File) => void;
}

export function CloudStorageButtons({
  variant = "default",
  onFileSelect,
}: CloudStorageButtonsProps) {
  const [picker, setPicker] = useState<CloudProvider | null>(null);

  const openPicker = (provider: CloudProvider) => {
    if (!onFileSelect) return;
    setPicker(provider);
  };

  return (
    <>
      {variant === "compact" ? (
        <div className="flex items-center justify-center gap-4">
          {STORAGE_PROVIDERS.map(({ id, label, Icon }) => (
            <button
              key={id}
              type="button"
              aria-label={`Import from ${label}`}
              onClick={(e) => {
                e.stopPropagation();
                openPicker(id);
              }}
              className={cn(
                "flex h-11 w-11 items-center justify-center rounded-full border border-[#d5d5d5] bg-white transition-colors hover:border-[#bdbdbd] hover:bg-[#fafafa] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1473e6] focus-visible:ring-offset-2 dark:border-slate-600 dark:bg-slate-900",
                !onFileSelect && "cursor-not-allowed opacity-50"
              )}
            >
              <Icon className="h-6 w-6" />
            </button>
          ))}
        </div>
      ) : (
        <div className="flex flex-wrap items-center justify-center gap-3">
          {STORAGE_PROVIDERS.map(({ id, label, Icon }) => (
            <button
              key={id}
              type="button"
              aria-label={`Import from ${label}`}
              onClick={(e) => {
                e.stopPropagation();
                openPicker(id);
              }}
              className={cn(
                "group inline-flex min-h-11 min-w-[11rem] items-center gap-3 rounded-full border border-[var(--ge-border)] bg-white px-4 py-2.5 text-left shadow-sm transition-all hover:border-[#c8c8c8] hover:bg-[var(--ge-surface)] hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-blue)] focus-visible:ring-offset-2 active:scale-[0.98]",
                !onFileSelect && "cursor-not-allowed opacity-50"
              )}
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--ge-surface)] transition-colors group-hover:bg-white">
                <Icon className="h-[22px] w-[22px]" />
              </span>
              <span className="text-[13px] font-medium text-[var(--ge-text)]">{label}</span>
            </button>
          ))}
        </div>
      )}

      {picker && onFileSelect ? (
        <CloudFilePicker
          provider={picker}
          open
          onClose={() => setPicker(null)}
          onFileSelect={onFileSelect}
        />
      ) : null}
    </>
  );
}
